...
export default function Navbar() {
  ...
  return (
    <nav style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: "80px" }}>
          <Link href="/" style={{ display: "flex", alignItems: "center", gap: "12px", textDecoration: "none" }}>
            ...
            <div className="hidden-mobile">
              <div style={{ color: "white", fontWeight: 700, fontSize: "1.05rem", lineHeight: 1.2, fontFamily: "'Playfair Display', serif" }}>Frigate Logistics</div>
              <div style={{ color: "#D4A843", fontSize: "10px", fontWeight: 600, letterSpacing: "0.15em", textTransform: "uppercase" }}>Ltd</div>
            </div>
          </Link>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link key={item.label} href={item.href} style={{
                  ...
                  textDecoration: "none", transition: "color 0.3s", paddingBottom: "4px",
                }}
                ...
                onMouseLeave={e => { if (!isActive) (e.currentTarget as HTMLElement).style.color = "#C0C8D8"; }}>
                  {item.label}
                  <span style={{ position: "absolute", bottom: 0, left: 0, width: isActive ? "100%" : "0", height: "1px", background: "#D4A843", transition: "width 0.3s" }} />
                </Link>
              );
            })}
          </div>
          ...
          <button className="mobile-toggle" onClick={() => setMenuOpen(!menuOpen)} style={{ background: "none", border: "none", color: "#C0C8D8", cursor: "pointer", padding: "8px" }}>
            {menuOpen ? (
      ...
      <div style={{ maxHeight: menuOpen ? "320px" : "0", overflow: "hidden", transition: "max-height 0.3s ease", background: "rgba(10,22,40,0.98)", borderTop: menuOpen ? "1px solid rgba(44,74,110,0.4)" : "none" }}>
        <div style={{ padding: "16px 32px 24px", display: "flex", flexDirection: "column", gap: "4px" }}>
          {navItems.map((item) => (
            <Link key={item.label} href={item.href} style={{ color: pathname === item.href ? "#D4A843" : "#C0C8D8", padding: "12px 0", fontSize: "15px", fontWeight: 500, borderBottom: "1px solid rgba(44,74,110,0.3)", textDecoration: "none" }}>{item.label}</Link>
          ))}
          <Link href="/contact" className="btn-primary" style={{ marginTop: "16px", justifyContent: "center" }}>Get a Quote</Link>
        </div>
      </div>

      <style>{`
        @media (max-width: 768px) { .desktop-nav, .desktop-cta, .hidden-mobile { display: none !important; } }
